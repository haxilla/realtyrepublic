...
->hasTable('propremarkSynch')){
  $results=DB::select( DB::raw("
    create table remailsynch.propremarkSynch
    like propremarks
  "));};
...
->dropIfExists('propremarks_federated');
//create federated table
//create table with identical field names
$results=DB::select( DB::raw("
  CREATE TABLE remailsynch.propremarks_federated (
    ufid          int,
    umid          int,
    remarkLink    boolean,
    e_remarks     text,
...
Schema::connection('remailsynch')
->dropIfExists('propremarkBackup');
//create propagentSynch Table
$results=DB::select( DB::raw("
  create table remailsynch.propremarkBackup
  like propremarks
"));
//  ** insert backup copy
$results = DB::select( DB::raw("
    INSERT INTO remailsynch.propremarkBackup
    SELECT *
    FROM propremarks
"));
// ** drop original
...
$results=DB::select( DB::raw("
  ...
  like remailsynch.propremarkBackup
"));
...
SELECT
  ...
  b7,
  b8
FROM remailsynch.propremarks_federated
"));
...
//insert
$results = DB::select( DB::raw("
    INSERT INTO remailsynch.propremarkSynch
    SELECT *
    FROM propremarks
"));
...
$idArray = array(
  'status'          => 'success',
  'next'            => 'resetStat',
  'message1'        => 'propRemarks Reset!',
  'message2'        => 'Now resetting propflyerstats...'
);
