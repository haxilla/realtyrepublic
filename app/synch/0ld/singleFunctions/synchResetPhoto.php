...
//startup synch table
if(!Schema::hasTable('propphotoSynch')){
  $results=DB::select( DB::raw("
    create table propphotoSynch
    like propphotos
  "));
  //insert
  $results = DB::select( DB::raw("
      INSERT INTO propphotoSynch
      SELECT *
      FROM propphotos
  "));
};
...
$results=DB::select( DB::raw("
  CREATE TABLE remailphotos_federated (
...
$results=DB::select( DB::raw("
  create table propphotoBackup
  like propphotos
"));
//insert
$results = DB::select( DB::raw("
    INSERT INTO propphotoBackup
    SELECT *
    FROM propphotos
"));
//drop
...
SELECT
  ...
  localFound,
  remoteFound
FROM remailphotos_federated
"));

include('synchResetPhotoCombined.php');
