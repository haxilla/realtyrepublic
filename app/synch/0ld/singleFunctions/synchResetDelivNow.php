...
//table check
if(!Schema::hasTable('propdelivnowSynch')){
  $results=DB::select( DB::raw("
    create table propdelivnowSynch
    like propdelivs
  "));
};
...
$results=DB::select( DB::raw("
  CREATE TABLE remaildeliveriesnow_federated (
    ufid                INT,
...
$results=DB::select( DB::raw("
  create table propdelivnowBackup
  like propdelivnow
"));
//insert
$results = DB::select( DB::raw("
    INSERT INTO propdelivnowBackup
    SELECT *
    FROM propdelivnow
"));
//drop
...
DB::select( DB::raw("
  ...
  SELECT
    ...
    server,
    free
  FROM  remaildeliveriesnow_federated
"));
//2nd backup
//delete if exists
