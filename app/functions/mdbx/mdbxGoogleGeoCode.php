...
$checkGoogleGEO=propmapping::select('googlat','googlng')
->where('propflyer_id','=',"$newID")
->where('propagent_id','=',"$umid")
->first();

//if any records its ok to check and map
if(!$checkGoogleGEO['googlat'] || !$checkGoogleGEO['googlng']){
...
//check if it exists
$getMap=propmapping::where('propflyer_id','=',"$newID")
->first();
...
if($getMap){
   if(!$getMap->googlat){
      ...
      propmapping::where('propflyer_id','=', "$newID")
      ->update([
