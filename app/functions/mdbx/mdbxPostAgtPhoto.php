<?php
$target_dir = "hqoffice/$officeID/";
$target_file = $target_dir . uniqid() .'-'. basename($_FILES["agtPhoto"]["name"]);
$newFileName=basename($target_file);
$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);

//if directory doesnt exist create it
 if (!is_dir("hqoffice/$officeID")) {
     mkdir("hqoffice/$officeID", 0777, true);
 }

// Check if image file is a actual image or fake image
if(isset($_POST["submit"])) {
    $check = getimagesize($_FILES["agtPhoto"]["tmp_name"]);
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["agtPhoto"]["tmp_name"], $target_file)) {
