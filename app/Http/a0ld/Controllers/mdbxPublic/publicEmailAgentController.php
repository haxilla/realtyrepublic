...
class publicEmailAgentController extends Controller
{
   public function postEmailAgentModal(Request $request){
      //request
      $ajid=request('ajid');
      //default
      $status='success';
      ...
      if($success==false){
        ...
        return response()->json(['errors'=>'Sorry we had an issue with that request, please try again!']);}

      //set vars
      $modalName=$_POST['modalName'];
      $modalEmail=$_POST['modalEmail'];
      $modalMessage=$_POST['modalMessage'];
      //insert into memberMessages & sendEmail to admin first
      remailmsg::create([
         'propagent_id' => $umid,
         'propflyer_id' => 0,
         'msg_date'     =>\Carbon\Carbon::now(),
         'fromForm'     => 'contactModal',
         'ip'           => \Request::ip(),
         'senderName'   => $modalName,
         'senderEmail'  => $modalEmail,
         'msg'          => $modalMessage,
      ]);
   ...
   public function emailAgentPost(Request $request){
      ...
      */
      ...
      //send admin msg
      $toEmail    = 'ins-6hgvgpit@isnotspam.com';
      $toName     = 'Mark Smith';
      $agtFirst   = $toName;
